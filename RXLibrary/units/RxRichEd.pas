...
{$IFDEF RX_ENH}
    property LinesUpdating: Boolean read FLinesUpdating write FLinesUpdating;//ïðè ñáîðêå ñòðîêè èç êóñî÷êîâ, ÷òîáû íå ìåðöàëî âûäåëåíèå
    property RtfSelText: String read GetRtfSelText write SetRtfSelText;//âûäåëåíûé òåêñò â native ôîðìàòå
    property LinkClickRange: TCharRange read FClickRange;//+GetTextRange íà ÷¸ì êëèêíóëè
{$ENDIF}
  {$IFDEF RX_ENHPRINT}
...
{.$DEFINE PURE_PASCAL}
{$IFDEF RX_D12}
function AdjustLineBreaks(Dest: PByte; const Source: TBytes; Start, Len: Integer): Integer;
var
  I: Integer;
  P: PByte;
begin
  ...
  begin
    // Zpracování CR+LF (0D 00 0A 00 v hex pro UTF-16LE)
    if (I <= Start + Len - 4) and (Source[I] = $0D) and (Source[I+1] = $00) and
       (Source[I+2] = $0A) and (Source[I+3] = $00) then
    begin
      // Kopírování CR+LF jako je
      P^ := Source[I]; Inc(P);
      P^ := Source[I+1]; Inc(P);
      Inc(I, 4); // Pøeskoèení pøes CR+LF
    end
    // Zpracování samostatného CR nebo LF
    else if (I <= Start + Len - 2) and ((Source[I] = $0D) or (Source[I] = $0A)) and (Source[I+1] = $00) then
    begin
      // Pøidání CR (0D 00)
      P^ := $0D; Inc(P);
      P^ := $00; Inc(P);
      // Pøidání LF (0A 00) ihned po CR
      P^ := $0A; Inc(P);
      P^ := $00; Inc(P);
      Inc(I, 2); // Pøeskoèení pøes CR nebo LF
    end
    else
    begin
      // Kopírování ostatních bajtù
      P^ := Source[I]; Inc(P);
      Inc(I);
      if I < Start + Len then
      begin
  ...
  // Výpoèet a vrácení poètu zapsaných bajtù
...
{$ELSE}
function AdjustLineBreaks(Dest, Source: PChar): Integer;
var
  PDest: PChar;
  PSource: PChar;
begin
  ...
  // Kopírování znakù z jednoho øetìzce do druhého
  while PSource^ <> #0 do
  begin
    // Pokud je aktuální znak nový øádek, vložíme za ním CRLF
    if PSource^ = #13 then
    begin
      PDest^ := PSource^;
      Inc(PDest);
    ...
    begin
      // Kopírování aktuálního znaku do cílového øetìzce
      PDest^ := PSource^;
      Inc(PDest);
      Inc(PSource);
    end;
  ...
  // Výpoèet délky upraveného øetìzce
  Result := Integer(PDest) - Integer(Dest);
end;
{$ENDIF}
{$ENDIF}

{$IFDEF RX_D12}
function StreamSave(dwCookie: {$IFDEF RX_D19}DWORD_PTR{$ELSE}Longint{$ENDIF}; pbBuff: PByte;
...
{$IFDEF RX_D12}
function StreamLoad(dwCookie: {$IFDEF RX_D19}DWORD_PTR{$ELSE}Longint{$ENDIF}; pbBuff: PByte;
    cb: Longint; var pcb: Longint): Longint; stdcall;
var
